  ...
  return (
    <div className="p-6 lg:p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h2 className="text-sm font-semibold text-slate-700 mb-4">Pipeline Summary</h2>
          <div className="grid grid-cols-2 gap-4 mt-2">
            <SummaryCard
              label="Total Topics"
              value={topics.filter((t) => t.status !== 'discarded').length}
            />
            <SummaryCard
              label="With Keyword Data"
              value={topics.filter((t) => t.status !== 'discarded' && t.search_volume != null).length}
            />
            <SummaryCard
              label="In Opportunity Zone"
              value={topics.filter((t) => t.status !== 'discarded' && (t.keyword_difficulty ?? 100) < 40 && (t.search_volume ?? 0) > 200).length}
              highlight
            />
            <SummaryCard
              ...
              value={(() => {
                ...
                return Math.round(active.reduce((sum, t) => sum + t.keyword_difficulty, 0) / active.length);
              })()}
            />
            <SummaryCard
              label="Avg. Volume"
              value={(() => {
                const active = topics.filter((t) => t.status !== 'discarded' && t.search_volume != null);
                if (!active.length) return '—';
                return Math.round(active.reduce((sum, t) => sum + t.search_volume, 0) / active.length).toLocaleString();
              })()}
            />
            <SummaryCard
