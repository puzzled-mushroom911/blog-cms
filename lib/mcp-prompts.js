...
Welcome to the Blog CMS — a lightweight, AI-powered content management system built on Supabase. It gives you CRUD tools for blog posts, a topic research pipeline, and a style preference system that learns from your edits over time.
...
- **Manage a content pipeline** — topics flow from researched -> approved -> writing -> written
- **Learn your voice** — the feedback loop captures your edits and applies them to future posts
- **Publish anywhere** — Supabase-powered sites read directly from the DB; WordPress sites get published via API
...
Use \`create_post\` with at minimum a title. The content field takes an array of content blocks:
...
- **publish-to-wordpress** — Configure and use WordPress publishing
...
After generating the content blocks, use the create_post tool to save the post:
- Set content to the JSON array of blocks
- Set status to "draft" for review
- Include title, slug, excerpt, meta_description, category, tags, and keywords`;
...
const PUBLISH_TO_WORDPRESS = `# Publish to WordPress

Configure and use WordPress publishing from the Blog CMS to push content directly to your WordPress site.

## Overview

The CMS can publish blog posts directly to WordPress via the WordPress REST API. Content blocks are automatically converted to HTML. No WordPress plugins required — it uses the built-in Application Passwords feature.
...
4. Enter a name (e.g., "Blog CMS") and click "Add New Application Password"
5. Copy the generated password (you won't see it again)

**Note:** Application Passwords require WordPress 5.6+ and HTTPS. If you don't see the option, check that your site uses HTTPS.

## Step 2: Configure WordPress in the CMS

In the CMS Settings page, add your WordPress connection:

- **Site URL** — Your WordPress site URL (e.g., https://example.com)
- **Username** — Your WordPress username
- **Application Password** — The password generated in Step 1

These credentials are stored in your workspace settings. Use the "Test Connection" button to verify.

## Step 3: Publish a Post

1. Create and edit your blog post in the CMS as usual
2. Set the post status to "published" using the publish_post tool
3. Click "Publish to WordPress" (or use the API endpoint)
4. The CMS converts your content blocks to WordPress HTML and sends it to your site

## Content Block Conversion

Your CMS content blocks are automatically converted:

| CMS Block | WordPress HTML |
|-----------|---------------|
| paragraph | \`<p>text</p>\` |
| heading | \`<h2>text</h2>\` |
...
| pros-cons | Two-column styled \`<div>\` |
| info-box | Styled \`<div>\` |
| stat-cards | Styled grid \`<div>\` |
| process-steps | \`<ol>\` with styled steps |
| prompt | Skipped (internal CMS block) |
...
Returns the WordPress post URL on success.

## Tips

- Posts are sent as **drafts** to WordPress by default — review and publish from the WordPress dashboard if you prefer
- Categories and tags from the CMS are passed to WordPress (created automatically if they don't exist yet)
- Featured images are referenced by URL — they are not uploaded to the WordPress media library
- To re-publish after edits, use the same endpoint — it will update the existing WordPress post

## Troubleshooting

- **401 Unauthorized** — Check your username and application password. Make sure your site uses HTTPS.
- **404 Not Found** — Verify the site URL includes the protocol (https://) and has no trailing slash.
- **rest_cannot_create** — Your WordPress user needs Editor or Administrator role.
- **Connection refused** — Some hosts block REST API access. Check with your hosting provider.`;
...
const PROMPTS = [
  ...
  },
  {
    ...
    description: 'Configure and use WordPress publishing — set up Application Passwords, connect your site, and publish posts directly from the CMS.',
    content: PUBLISH_TO_WORDPRESS,
  },
];
