...
/**
 * Shared tool definitions for both MCP servers (stdio + HTTP).
 * Single source of truth — no duplication between bin/mcp-server.js and api/mcp.js.
 */
...
/**
 * Registers all 13 CMS tools on the given McpServer instance.
 *
 * @param {import('@modelcontextprotocol/sdk/server/mcp.js').McpServer} server
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} workspace_id
 */
export function registerAllTools(server, supabase, workspace_id) {
  ...
  server.registerTool(
    'create_post',
    {
      ...
      inputSchema: z.object({
        title: z.string().max(200).describe('Post title (max 200 chars)'),
        ...
        read_time: z.string().max(20).optional().describe('e.g. "5 min read"'),
        author: z.string().max(100).optional().describe('Author name'),
        category: z.string().max(50).optional().describe('General, How-To, Guide, Review, News, Tips, Case Study, Comparison'),
        tags: z.array(z.string().max(50)).max(20).optional().describe('Tag strings (max 20 tags)'),
        youtube_id: z.string().max(20).optional().describe('YouTube video ID'),
        image: z.string().max(2000).optional().describe('Featured image URL'),
        meta_description: z.string().max(320).optional().describe('SEO meta description'),
        keywords: z.string().max(500).optional().describe('SEO keywords'),
        content: z.array(z.record(z.any())).max(200).optional().describe('Array of content block objects (max 200)'),
        ...
        sources: z.array(z.object({
          ...
          note: z.string().max(500).optional().describe('Why this source matters'),
        })).max(50).optional().describe('Research sources (max 50)'),
        ai_reasoning: z.string().max(5000).optional().describe('How the AI researched and structured the content'),
      }),
    },
    async (args) => {
